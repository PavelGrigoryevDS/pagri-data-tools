...
def box(
    ...
    **kwargs
    ...
    sort : bool, optional
        If True, sorts categories by median. Default is True.
    ...
    **kwargs :
    ...
    def prepare_df(config, kwargs):
        data_frame = config['data_frame']
        top_n_trim = config['top_n_trim']
        num_col = config['num_col']
        cat_col = config['cat_col']
        agg_func_for_top_n = config['agg_func_for_top_n']
    ...
    **kwargs : dict
    ...
    def _make_kde_trace(trace, config):
        ...
        if len(kde_data) < 2:
            ...
            y_values_kde = kde_obj(x_values_kde).round(7)
    ...
    def _make_subplots_fig(data, config, kwargs):
        ...
        if show_box:
            if 'color' in kwargs:
                ...
                row_heights_box = 0.07
                row_heights_hist = 0.93
            config['row_heights_box'] = row_heights_box
            config['row_heights_hist'] = row_heights_hist
        ...
        for trace in data:
            ...
            if col == 1:
                subplots_fig.update_yaxes(
                    title_text=config.get('yaxis_title')
            ...
            if row_hist == last_row:
                subplots_fig.update_xaxes(
                    title_text=config.get('xaxis_title')
                    , row=row_hist, col=col
                )
            ...
            if show_box:
                ...
                if config['data_frame'] is not None:
                    if kwargs.get('labels') is not None and config['num_col'] in kwargs['labels']:
                ...
                subplots_fig.update_xaxes(
                    showticklabels=False, showline=False, ticks='', showgrid=True, row=row_box, col=col
                )
                subplots_fig.update_yaxes(
    ...
    def _update_fig(fig, config, kwargs):
        ...
        layout_updates = {
            ...
            'height': kwargs.get('height'),
            'width': kwargs.get('width'),
            'margin': dict(l=50, r=50, b=50, t=90) if 'color' in kwargs else dict(l=50, r=50, b=50, t=50),
        }
        # Update layout only if there are updates
        layout_updates = {k: v for k, v in layout_updates.items() if v is not None}
        if layout_updates:
        ...
        legend_updates = {
            ...
            'legend_x': 0.5,
            'legend_y': 1.11 if 'color' in kwargs else 1.1 ,
...
def sentiment_analysis(df, text_column, method='vader', clean_text=True):
    """
    ...
    method (str): The method to use for sentiment analysis ('textblob' or 'vader').
    clean_text (bool): Whether clean text

    Returns:
    fig: A Plotly figure object representing the histogram of sentiment polarity.
    """
    # Загрузка VADER лексикона только при первом вызове функции
    if method == 'vader':
        nltk_download('vader_lexicon', quiet=True)  # quiet=True, чтобы не выводить сообщения
    ...
    def analyze_sentiment(text):
        ...
        if method == 'textblob':
            ...
            return sia.polarity_scores(text)['compound']  # Используем compound для VADER
